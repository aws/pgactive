...
To make diagnosis and handling of active-active conflicts easier, BDR supports logging of each conflict incident in a [`bdr.bdr_conflict_history`](catalog-bdr-conflict-history.md) table.
Conflict logging to this table is only enabled when [bdr.log_conflicts_to_table](bdr-configuration-variables.md#GUC-BDR-LOG-CONFLICTS-TO-TABLE) is `true` BDR also logs conflicts to the PostgreSQL log file if `log_min_messages` is `LOG `or lower, irrespective of the value of `bdr.log_conflicts_to_table`
