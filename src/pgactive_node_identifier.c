...
{
	int			i;
	pgactiveNodeIdentifier *found_w = NULL;
...
{
	pgactiveNodeIdentifier *w;

	LWLockAcquire(pgactiveNodeIdentifierCtl->lock, LW_EXCLUSIVE);
	w = pgactive_get_nid_shmem_pointer(dboid);
	if (!w)
		ereport(ERROR,
...
pgactive_get_data_only_node_init(Oid dboid)
{
	...
	data_only_node_init = w->data_only_node_init;
	LWLockRelease(pgactiveNodeIdentifierCtl->lock);
