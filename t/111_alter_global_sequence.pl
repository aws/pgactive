...
qq{ SELECT bdr.bdr_snowflake_id_nextval('test_seq'::regclass) FROM generate_series(1,3::bigint); });
...
($ret, $stdout, $stderr) = $node_a->psql($bdr_test_dbname,
qq{ SELECT bdr.bdr_snowflake_id_nextval('test_seq'::regclass) FROM generate_series(1,4::bigint); });
