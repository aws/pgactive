...
sub pgactive_detach_join_concurrency_tests {
    ...
    # Detach two nodes concurrently
    concurrent_detach( \@{ [ $node_f, $upstream_node ] }, \@{ [ $node_g, $upstream_node ] });
    ...
    # Three concurrent detach
    concurrent_detach( \@{ [ $node_h, $upstream_node ] }, \@{ [ $node_i, $upstream_node ] }, \@{ [ $node_j, $upstream_node ] });
    stop_nodes( [ $node_h, $node_i, $node_j ] );

    note "Concurrent detach and join\n";

    # Concurrent detach and join.
    ...
    concurrent_join_detach( $type, $upstream_node, [ $node_l, $node_m ], [$node_k] );

    note "Clean up\n";
    detach_and_check_nodes( [ $node_l, $node_m ], $upstream_node );
    stop_nodes( [$node_k] );
    
    note "Concurrent detach from different upstream\n";
    # Join a new node to create a 2 node cluster
    my $node_b = PostgreSQL::Test::Cluster->new('node_b');
    initandstart_join_node( $node_b, $node_a, $type );

    # detach nodes concurrently from different upstreams
    ...
    concurrent_detach( \@{ [ $node_P, $node_a ] }, \@{ [ $node_Q, $node_b ] } );
