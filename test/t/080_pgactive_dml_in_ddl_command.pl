...
#
# pgactive 2.0 has significant limiations when mixing DDL and DML. The lack of full
# DDL deparse means we have to do statement-based replication for DDL. If there's
# DML mixed in with DDL in a pgactive.pgactive_replicate_ddl_command or multi-statement,
# we'll queue both for replay. But since we'd also replay the DML directly,
# we refuse.
#
# https://github.com/2ndQuadrant/pgactive-private/issues/23
#
...
# pgactive.do_not_replicate=true and used WAL messages to log the DDL, but it seems
...
$node_a->safe_psql($pgactive_test_dbname,"INSERT INTO $table_name VALUES(1),(2)");

# Join a new node to first node using pgactive_join_group
my $node_b = PostgreSQL::Test::Cluster->new('node_b');
initandstart_logicaljoin_node($node_b,$node_a);

#  pgactive.pgactive_replicate_ddl_command(...) disallows DML (INSERT/UPDATE/DELETE).
#  ... or it should. TODO.

TODO: {
    local $TODO = 'pgactive_replicate_ddl_command needs to filter out DML?';

    my ($dml_statement, $error_msg);

    # INSERT test
    $dml_statement = qq{SELECT pgactive.pgactive_replicate_ddl_command(\$DDL\$ INSERT INTO public.$table_name VALUES(3);\$DDL\$);};
    ...
    # UPDATE Test
    $dml_statement = qq{SELECT pgactive.pgactive_replicate_ddl_command(\$DDL\$ UPDATE public.$table_name SET id = 4 WHERE id = 1;\$DDL\$);};
    $error_msg = "Direct DML statements are not supported";
    dml_fail($node_b,$dml_statement,$error_msg);
    check_table($table_name,$node_a,$node_b);

    # DELETE Test
    $dml_statement = qq{SELECT pgactive.pgactive_replicate_ddl_command(\$DDL\$ DELETE FROM public.$table_name WHERE id = 2;\$DDL\$);};
    $error_msg = "Direct DML statements are not supported";
    dml_fail($node_b,$dml_statement,$error_msg);
    check_table($table_name,$node_a,$node_b);
};
...
sub dml_fail {
	...
	my ($ret, $stdout, $stderr) = $node->psql($pgactive_test_dbname,$dml_statement);
...
sub check_table {
	...
	foreach my $node (@nodes) {
		...
		my $table_values = $node->safe_psql($pgactive_test_dbname,"SELECT * FROM $table_name");
