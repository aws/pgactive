...
my $join_query = generate_pgactive_logical_join_query($node_g2_c2, $node_g1_c1);

# Must not use safe_psql since we expect an error here
my ($psql_ret, $psql_stdout, $psql_stderr) = ('','', '');
($psql_ret, $psql_stdout, $psql_stderr) = $node_g2_c2->psql(
    $bravo,
    $join_query);
like($psql_stderr, qr/.*ERROR.*this node is already a member of a pgactive group/,
     "joining of a node failed due to a database part of a pgactive group joining another pgactive group");
