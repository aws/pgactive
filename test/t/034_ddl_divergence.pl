...
for my $node ($node_a, $node_b)
{
	...
	$node->append_conf('postgresql.conf', q{pgactive.skip_ddl_replication = true});
	$node->start;
...
# create a pgactive group
create_pgactive_group($node_a);

# join the pgactive group
pgactive_logical_join($node_b, $node_a);
...
$node_a->safe_psql($pgactive_test_dbname, q{insert into trig_test values(1,1)});
wait_for_apply($node_a, $node_b);
is($node_b->safe_psql($pgactive_test_dbname, 'SELECT a,b FROM trig_test;'),
...
{
	$node->safe_psql(
		$pgactive_test_dbname, q{CREATE FUNCTION trigtest() returns trigger as $$
										 BEGIN
	...
	$node->safe_psql(
		$pgactive_test_dbname, q{CREATE TRIGGER test_tg BEFORE INSERT ON trig_test
...
wait_for_apply($node_b, $node_a);
# Insert and ensure it is replicated
$node_a->safe_psql($pgactive_test_dbname, q{insert into trig_test values(2,2)});
wait_for_apply($node_a, $node_b);
is( $node_b->safe_psql(
		$pgactive_test_dbname, 'SELECT a,b FROM trig_test2 where a = 2;'),
	'2|2',
	"insert due to remote before insert trigger");

# Create a trigger on one node only (should not be done!)
$node_b->safe_psql(
	$pgactive_test_dbname, q{CREATE FUNCTION trigtest2() returns trigger as $$
										 BEGIN
...
$node_b->safe_psql(
	$pgactive_test_dbname, q{CREATE TRIGGER test_tg2 BEFORE INSERT ON trig_test
										 FOR EACH ROW EXECUTE PROCEDURE trigtest2();
										 });

# check that the local trigger works
$node_b->safe_psql($pgactive_test_dbname, q{insert into trig_test values(3,3)});
is( $node_b->safe_psql(
		$pgactive_test_dbname, 'SELECT a,b FROM trig_test2 where a = 3;'),
	'3|3',
	"local before insert trigger");
is( $node_b->safe_psql(
		$pgactive_test_dbname, 'SELECT a,b FROM trig_test2 where a = 53;'),
	'53|153',
...
wait_for_apply($node_b, $node_a);
is( $node_a->safe_psql(
		$pgactive_test_dbname, 'SELECT a,b FROM trig_test where a = 3;'),
	'3|3',
	"insert replicated");
is( $node_a->safe_psql(
		$pgactive_test_dbname, 'SELECT a,b FROM trig_test2 where a = 3;'),
...
$node_a->safe_psql($pgactive_test_dbname, q{insert into trig_test values(4,4)});
wait_for_apply($node_a, $node_b);
is( $node_b->safe_psql(
		$pgactive_test_dbname, 'SELECT a,b FROM trig_test2 where a = 4;'),
	'4|4',
	"insert due to remote before insert trigger");
is( $node_b->safe_psql(
		$pgactive_test_dbname, 'SELECT count(*) FROM trig_test2 where a = 54;'),
	'0',
...
for my $node ($node_a, $node_b)
{
	...
	$node->safe_psql($pgactive_test_dbname, q{DROP TRIGGER test_tg on trig_test;});

	# Some cleaning
	$node->safe_psql($pgactive_test_dbname, q{TRUNCATE TABLE trig_test;});
	$node->safe_psql($pgactive_test_dbname, q{TRUNCATE TABLE trig_test2;});

	# Create the trigger
	$node->safe_psql(
		$pgactive_test_dbname, q{CREATE TRIGGER test_tg AFTER INSERT ON trig_test
										 FOR EACH ROW EXECUTE PROCEDURE trigtest();
										 });
}
...
$node_b->safe_psql($pgactive_test_dbname, q{DROP TRIGGER test_tg2 ON trig_test;});
...
# Create a trigger on one node only (should not be done!)
$node_b->safe_psql(
	$pgactive_test_dbname, q{CREATE TRIGGER test_tg2 AFTER INSERT ON trig_test
										 FOR EACH ROW EXECUTE PROCEDURE trigtest2();
										 });
...
for my $node ($node_a, $node_b)
{
	...
	# Create the trigger
	$node->safe_psql(
		$pgactive_test_dbname, q{CREATE TRIGGER test_tg BEFORE UPDATE ON trig_test
										 FOR EACH ROW EXECUTE PROCEDURE trigtest();
										 });
}
...
$node_a->safe_psql($pgactive_test_dbname, q{insert into trig_test values(2,2)});
$node_a->safe_psql($pgactive_test_dbname, q{update trig_test set a=2, b=2});
wait_for_apply($node_a, $node_b);
is( $node_b->safe_psql(
		$pgactive_test_dbname, 'SELECT a,b FROM trig_test2 where a = 2;'),
	'2|2',
...
# Create a trigger on one node only (should not be done!)
$node_b->safe_psql(
	$pgactive_test_dbname, q{CREATE TRIGGER test_tg2 BEFORE UPDATE ON trig_test
										 FOR EACH ROW EXECUTE PROCEDURE trigtest2();
										 });

# check that the local trigger works
$node_b->safe_psql($pgactive_test_dbname, q{update trig_test set a=3, b=3});
is( $node_b->safe_psql(
		$pgactive_test_dbname, 'SELECT a,b FROM trig_test2 where a = 3;'),
	'3|3',
...
# on the other way around check that local trigger is not triggered
$node_a->safe_psql($pgactive_test_dbname, q{update trig_test set a=4, b=4});
wait_for_apply($node_a, $node_b);
is( $node_b->safe_psql(
		$pgactive_test_dbname, 'SELECT a,b FROM trig_test2 where a = 4;'),
	'4|4',
...
for my $node ($node_a, $node_b)
{
	...
	# Create the trigger
	$node->safe_psql(
		$pgactive_test_dbname, q{CREATE TRIGGER test_tg AFTER UPDATE ON trig_test
										 FOR EACH ROW EXECUTE PROCEDURE trigtest();
										 });
}
...
# Create a trigger on one node only (should not be done!)
$node_b->safe_psql(
	$pgactive_test_dbname, q{CREATE TRIGGER test_tg2 AFTER UPDATE ON trig_test
										 FOR EACH ROW EXECUTE PROCEDURE trigtest2();
										 });
...
for my $node ($node_a, $node_b)
{
	...
	$node->safe_psql($pgactive_test_dbname, q{DROP FUNCTION trigtest();});
	...
	# Create the trigger
	$node->safe_psql(
		$pgactive_test_dbname, q{CREATE TRIGGER test_tg BEFORE DELETE ON trig_test
										 FOR EACH ROW EXECUTE PROCEDURE trigtest();
										 });
}
...
# Create a trigger on one node only (should not be done!)
$node_b->safe_psql($pgactive_test_dbname, q{DROP FUNCTION trigtest2();});
$node_b->safe_psql(
	$pgactive_test_dbname, q{CREATE FUNCTION trigtest2() returns trigger as $$
										 BEGIN
...
$node_b->safe_psql(
	$pgactive_test_dbname, q{CREATE TRIGGER test_tg2 BEFORE DELETE ON trig_test
										 FOR EACH ROW EXECUTE PROCEDURE trigtest2();
										 });
...
wait_for_apply($node_b, $node_a);
is( $node_a->safe_psql(
		$pgactive_test_dbname, 'SELECT count(*) FROM trig_test where a = 3;'),
	'0',
	"delete replicated");
is( $node_a->safe_psql(
		$pgactive_test_dbname, 'SELECT a,b FROM trig_test2 where a = 3;'),
...
for my $node ($node_a, $node_b)
{
	...
	# Create the trigger
	$node->safe_psql(
		$pgactive_test_dbname, q{CREATE TRIGGER test_tg AFTER DELETE ON trig_test
										 FOR EACH ROW EXECUTE PROCEDURE trigtest();
										 });
}
...
# Create a trigger on one node only (should not be done!)
$node_b->safe_psql(
	$pgactive_test_dbname, q{CREATE TRIGGER test_tg2 AFTER DELETE ON trig_test
										 FOR EACH ROW EXECUTE PROCEDURE trigtest2();
										 });
...
$node_a->safe_psql($pgactive_test_dbname, q{TRUNCATE TABLE trig_test2;});
is($node_a->safe_psql($pgactive_test_dbname, 'select count(*) FROM trig_test2'),
	'0', "local truncate");
wait_for_apply($node_a, $node_b);
is( $node_b->safe_psql(
		$pgactive_test_dbname, 'select count(*) > 0 FROM trig_test2'),
	't',
...
{
	$node->safe_psql(
		$pgactive_test_dbname, q{CREATE TABLE tab1(id  int PRIMARY KEY,
										                   id2 int UNIQUE,
														   name varchar(50));});

	$node->safe_psql(
		$pgactive_test_dbname, q{CREATE TABLE tab2(id2 int PRIMARY KEY,
										                   tab1id int,
														   tab1id2 int,
...
# Add a few rows
$node_b->safe_psql($pgactive_test_dbname, q{insert into tab1 values(1,1,'1');});
$node_b->safe_psql($pgactive_test_dbname, q{insert into tab1 values(2,3,'2');});
$node_b->safe_psql($pgactive_test_dbname, q{insert into tab2 values(1,1,1);});
wait_for_apply($node_b, $node_a);

# This insert should fail on node_b
my ($psql_ret, $psql_stdout, $psql_stderr) = ('', '', '');
($psql_ret, $psql_stdout, $psql_stderr) =
  $node_b->psql($pgactive_test_dbname, qq{insert into tab2 values(2,2,2);});
...
wait_for_apply($node_b, $node_a);
$node_a->safe_psql($pgactive_test_dbname, q{insert into tab2 values(2,2,2);});
