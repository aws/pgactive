...
foreach my $i (0..2)
{
    ...
    $node_1->safe_psql($bdr_test_dbname, q[SELECT * FROM bdr.bdr_wait_for_slots_confirmed_flush_lsn(NULL, NULL)]);
...
$node_0->safe_psql($bdr_test_dbname, q[SELECT * FROM bdr.bdr_wait_for_slots_confirmed_flush_lsn(NULL, NULL)]);
