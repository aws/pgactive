...
my $join_query = generate_pgactive_logical_join_query($new_node, $node_0);
...
$offline_node->start;
is($new_node->psql($pgactive_test_dbname,
	qq[SELECT pgactive.pgactive_wait_for_node_ready($PostgreSQL::Test::Utils::timeout_default)]), 0,
...
# Everything is happy?
$new_node->safe_psql($pgactive_test_dbname, q[SELECT pgactive.pgactive_acquire_global_lock('ddl_lock')]);
...
my $handle = start_pgactive_init_copy($new_physical_join_node, $node_0, $new_conf_file,[$timeout]);
ok($handle->finish, 'pgactive_init_copy finished without error');

$timedout = 0;
$new_physical_join_node->psql($pgactive_test_dbname,
	qq[SELECT pgactive.pgactive_wait_for_node_ready($PostgreSQL::Test::Utils::timeout_default)],
	timed_out => \$timedout, timeout => 10);
...
is($new_physical_join_node->psql($pgactive_test_dbname,
	qq[SELECT pgactive.pgactive_wait_for_node_ready($PostgreSQL::Test::Utils::timeout_default)]), 0,
