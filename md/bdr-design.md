...
Every BDR node must be ***online and reachable*** when an attempt to join a new node is made. Otherwise the join will hang indefinitely or fail. BDR is a mesh where every node must be able to communicate with every other node, and while it is tolerant of network partitions and interruptions all nodes need to know about every other node that exist. See [BDR mesh network](https://github.com/aws/abba-pg-bdr/main/md/technotes-mesh.md).
...
For `INSERT` vs `INSERT` conflicts, use of [Global sequences](https://github.com/aws/abba-pg-bdr/main/md/global-sequences.md) can completely prevent conflicts.
...
* BDR's logical replication implementation imposes some restrictions on supported DDL (see: [DDL replication](https://github.com/aws/abba-pg-bdr/main/md/ddl-replication.md)) that do not apply for physical replication
