...
Conflict logging to this table is only enabled when [bdr.log_conflicts_to_table](/md/bdr-configuration-variables.md#GUC-BDR-LOG-CONFLICTS-TO-TABLE) is `true` BDR also logs conflicts to the PostgreSQL log file if `log_min_messages` is `LOG `or lower, irrespective of the value of `bdr.log_conflicts_to_table`
...
* BDR's logical replication implementation imposes some restrictions on supported DDL (see: [DDL replication](/abba-pg-bdr/md/ddl-replication.md)) that do not apply for physical replication
